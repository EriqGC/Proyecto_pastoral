...
$evento = [];
if ($evento_id > 0) {
    $query = "SELECT * FROM PTL_EVENTOS WHERE id = $evento_id";
...
<div id="modificarEventoModal" class="ptl-event-modal">
    <div class="ptl-modal-content">
        <span class="ptl-close-modal">&times;</span>
        <h2 class="ptl-modal-title">Modificar Evento</h2>
        ...
        <form id="formModificarEvento" action="modificar_evento.php" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="pagina_origen" value="<?= htmlspecialchars($_SERVER['HTTP_REFERER'] ?? '') ?>">
                ...
                <div class="ptl-form-grid">
                    <div class="ptl-form-column">
                        ...
                        <select class="ptl-form-select" name="tipo" id="tipoEvento" required>
                            ...
                            <option value="Noticia" <?= ($evento['tipo'] ?? '') === 'Noticia' ? 'selected' : '' ?>>Noticia</option>
                        </select>

                        <label class="ptl-form-label">Nombre del Evento:</label>
                        <input class="ptl-form-input" type="text" name="nombre" value="<?= htmlspecialchars($evento['nombre'] ?? '') ?>" required>

                        <label class="ptl-form-label">Costo:</label>
                        <input class="ptl-form-input" type="number" name="costo" id="costoInput" value="<?= htmlspecialchars($evento['costo'] ?? '') ?>" required>

                        <label class="ptl-form-label">Recurrencia:</label>
                        <select class="ptl-form-select" name="recurrencia" id="recurrenciaEvento" required>
                        ...
                        <input class="ptl-form-input" type="date" name="fecha_inicio" value="<?= htmlspecialchars($evento['fecha_inicio'] ?? '') ?>" required>

                        <label class="ptl-form-label">Fecha Fin:</label>
                        <input class="ptl-form-input" type="date" name="fecha_fin" value="<?= htmlspecialchars($evento['fecha_fin'] ?? '') ?>" required>

                        <label class="ptl-form-label">Descripción:</label>
                        <textarea class="ptl-form-textarea" name="descripcion" required><?= htmlspecialchars($evento['descripcion'] ?? '') ?></textarea>
                    </div>
                    <div class="ptl-image-column">
                        ...
                        <select class="ptl-form-select" name="categoria" required>
                            <option value="Práctica Misionera" <?= ($evento['categoria'] ?? '') === 'Práctica Misionera' ? 'selected' : '' ?>>Práctica Misionera</option>
                            <option value="Misión" <?= ($evento['categoria'] ?? '') === 'Misión' ? 'selected' : '' ?>>Misión</option>
                            <option value="Otro" <?= ($evento['categoria'] ?? '') === 'Otro' ? 'selected' : '' ?>>Otro</option>
                        </select>

                        <label class="ptl-form-label">Carta Compromiso:</label>
                        <div class="ptl-file-upload">
                            <?php if (!empty($evento['carta_compromiso'])): ?>
                                <div class="ptl-current-file">
                                    <p>Archivo actual: <span class="ptl-filename"><?= htmlspecialchars($evento['carta_compromiso']) ?></span>
                                        <a href="../assets/docs/<?= htmlspecialchars($evento['carta_compromiso']) ?>"
                                            target="_blank" class="ptl-btn ptl-btn-secondary" style="padding: 2px 2px; display: inline-block;">
                                            Ver documento
                                        </a>
                                    </p>
                                    <p class="ptl-change-notice">(Selecciona un nuevo archivo solo si deseas cambiarlo)</p>
                                </div>
                            <?php else: ?>
                                <p class="ptl-required-notice">* Se requiere subir la carta compromiso (formato PDF)</p>
                            <?php endif; ?>
                            <input class="ptl-form-input" type="file" name="carta_compromiso" accept=".pdf" <?= empty($evento['carta_compromiso']) ? 'required' : '' ?>>
                        </div>

                        <div class="ptl-image-upload">
                            <?php if (!empty($evento['imagen'])): ?>
                                <div class="ptl-current-image">
                                    <p>Imagen actual:</p>
                                    <img src="../assets/img/<?= ($evento['tipo'] === 'Noticia' ? 'img_noticias/' : 'img_eventos/') . htmlspecialchars($evento['imagen']) ?>"
                                        alt="Imagen actual" class="ptl-preview-image">
                                    <p class="ptl-filename"><?= htmlspecialchars($evento['imagen']) ?></p>
                                    <p class="ptl-change-notice">(Selecciona una nueva imagen solo si deseas cambiarla)</p>
                                </div>
                            <?php else: ?>
                                <p class="ptl-required-notice">* Se requiere subir una imagen para este evento</p>
                            <?php endif; ?>

                            <input class="ptl-form-input" type="file" name="imagen" accept="image/*" <?= empty($evento['imagen']) ? 'required' : '' ?>>
                        </div>
                        <img id="previewImagen" class="ptl-preview-image" src="../assets/img/<?= ($evento['tipo'] === 'Noticia' ? 'img_noticias/' : 'img_eventos/') . htmlspecialchars($evento['imagen']) ?>" alt="Vista previa">
                    </div>
                </div>
            </div>

            <!-- Itinerario -->
            <div id="itinerario" class="ptl-tab-content">
                <div class="ptl-itinerario-container">
                    <div id="ptl-hora-groups-container">
                        <?php foreach ($itinerario as $index => $item): ?>
                            <div class="ptl-hora-group">
                                <label class="ptl-form-label">Hora:</label>
                                <input class="ptl-form-input" type="time" name="hora_inicio[]" value="<?= htmlspecialchars($item['hora']) ?>" required>
                                <label class="ptl-form-label">Actividad:</label>
                                <input class="ptl-form-input" type="text" name="actividad[]" value="<?= htmlspecialchars($item['actividad']) ?>" required>
                                <?php if ($index > 0): ?>
                ...
                <div class="ptl-form-grid">
                    <div class="ptl-form-column">
                        ...
                        <select class="ptl-form-select" name="viaje" id="viajeSelect" required>
                            <option value="Si" <?= ($evento['ubicacion_final'] ?? '') !== '' ? 'selected' : '' ?>>Sí</option>
                            <option value="No" <?= ($evento['ubicacion_final'] ?? '') === '' ? 'selected' : '' ?>>No</option>
                        </select>

                        <label class="ptl-form-label">Ubicación Inicial:</label>
                        <input class="ptl-form-input" type="text" id="origen" name="origen" value="<?= htmlspecialchars($evento['ubicacion'] ?? '') ?>" placeholder="Ej: Ciudad de México">

                        <label class="ptl-form-label">Ubicación Destino:</label>
                        <input class="ptl-form-input" type="text" id="destino" name="destino" value="<?= htmlspecialchars($evento['ubicacion_final'] ?? '') ?>" placeholder="Ej: Guadalajara">

                        <label class="ptl-form-label">Vehículo:</label>
                        <select class="ptl-form-select" name="vehiculo" id="vehiculoSelect">
                            <option value="">Seleccione un vehículo</option>
                            <?php foreach ($vehiculos as $vehiculo): ?>
                                <option value="<?= htmlspecialchars($vehiculo) ?>" <?= ($evento['vehiculo'] ?? '') === $vehiculo ? 'selected' : '' ?>><?= htmlspecialchars($vehiculo) ?></option>
                            <?php endforeach; ?>
                            <option value="otro" <?= !in_array($evento['vehiculo'] ?? '', $vehiculos) && !empty($evento['vehiculo']) ? 'selected' : '' ?>>Otro (especificar)</option>
                        </select>
                        <input class="ptl-form-input" type="text" name="vehiculo_otro" id="vehiculoOtroInput"
                            style="display: <?= (!in_array($evento['vehiculo'] ?? '', $vehiculos) && !empty($evento['vehiculo'])) ? 'block' : 'none' ?>; margin-top: 5px;"
                            placeholder="Especificar vehículo"
                            value="<?= (!in_array($evento['vehiculo'] ?? '', $vehiculos) && !empty($evento['vehiculo'])) ? htmlspecialchars($evento['vehiculo']) : '' ?>">
                        <input class="ptl-form-input" type="number" name="asientos" id="asientosInput"
                            style="display: <?= (!in_array($evento['vehiculo'] ?? '', $vehiculos) && !empty($evento['vehiculo'])) ? 'block' : 'none' ?>; margin-top: 5px;"
                            placeholder="Número de asientos"
                            value="<?= $evento['asientos'] ?? '' ?>">
                    </div>
                    <div class="ptl-form-column">
...
<script>
    ...
    document.getElementById('modificarEventoModal').addEventListener('shown', function() {
        initMap();
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        tipoEvento.addEventListener('change', function() {
            ...
            document.getElementById('recurrenciaEvento').disabled = isNoticia;
        ...
        document.querySelector('.ptl-close-modal').addEventListener('click', function() {
            document.getElementById('modificarEventoModal').style.display = 'none';
        });
    ...
    function abrirModalModificacion(eventoId) {
        // Cargar el modal con el ID del evento
        const modal = document.getElementById('modificarEventoModal');

        // Puedes usar fetch para cargar el modal dinámicamente o:
        window.location.href = `?id=${eventoId}#modificarEventoModal`;

        // Mostrar el modal
        modal.style.display = 'block';

        // Si usas el método de hash, también necesitarás esto:
